...
public enum InputEventType {
	...
	GCTouchpad,		///Touchpad event that occured on a controller (Valve Steam Controller, Dualsense, etc.)
	Pen,
	DeviceAdded,
	DeviceRemoved,
	Clipboard,
...
public enum InputInitializationStatus {
	AllOk					=	0,
	win_RawInputError		=	-1,
	win_DevicesAdded		=	-2,		/// Devices added during initialization
	win_GIUnsupported		=	-3,		/// GameInput unsupported
	win_GIError				=	-4,		/// GameInput error

	x11_InputExtNotAvailable=	-32,
	libevdev_ErrorOpeningDev=	-33,	/// Error when opening device
	libevdev_AccessDenied	=	-34,	/// Access to device files not granted
}
/** 
...
/**
 * Defines a touchpad event.
 */
public struct TouchpadEvent {
	ubyte		dir;		///Up (0), or down (1)
	ubyte		num;		///Identifier number of fingers if multitouch is supported
	ushort		aux;		///Auxilliary value, normally 0, or 0-65_535 for touch pressue detection
	uint		touchPadID;	///Identifier of the touchpad on the device
	float		x;			///X coordinate of the event, between -1.0 and 1.0
	float		y;			///Y coordinate of the event, between -1.0 and 1.0
	float		xD;			///X difference of the touch event
	float		yD;			///Y difference of the touch event
}
/**
 * Defines a window resize or move event.
 */
